...
const ExcelGenerator = () => {
    ...
    try {
      const endpoint =
        datasetType === "expenses"
          ? "/excel/expenses/download"
          : "/excel/sales/download";

      const response = await apiClient.get(endpoint, {
        params: { count: recordCount },
        responseType: "blob",
      });
      ...
      link.href = url;
      link.download = `${datasetType}_report_${recordCount}_records_${
      ...
      toast.success(
        `${
          datasetType === "expenses" ? "Expense" : "Sales"
        } Excel file with ${recordCount} records downloaded successfully!`
      );
    } catch (error) {
  ...
  const loadStats = async () => {
    ...
    try {
      const count = Math.min(recordCount, 1000);
      const response =
        datasetType === "expenses"
          ? await api.excel.getExpenseStats(count)
          : await api.excel.getSalesStats(count);
    ...
    <div className="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        ...
        >
          <h3
            className={`text-xl font-semibold ${
              datasetType === "expenses" ? "text-blue-800" : "text-green-800"
            } mb-4`}
          >
          ...
          <button
            ...
            } text-white py-3 px-6 rounded-md disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors font-medium`}
          >
            {isGenerating ? (
        ...
        >
          <h3
            className={`text-xl font-semibold ${
              datasetType === "expenses" ? "text-green-800" : "text-blue-800"
            } mb-4`}
          >
            Preview Statistics
          </h3>
          ...
          <button
            ...
            className={`w-full ${
              datasetType === "expenses"
                ? "bg-green-600 hover:bg-green-700"
                : "bg-blue-600 hover:bg-blue-700"
            } text-white py-3 px-6 rounded-md disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors font-medium mb-4`}
          >
            {isLoadingStats ? (
              <span className="flex items-center justify-center">
                <svg
