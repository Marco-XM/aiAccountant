...
const Chatbot = () => {
  ...
  const loadChat = async (chatId) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/chat/sessions/${chatId}`, axiosConfig);
      setCurrentChatId(chatId);
      setMessages(response.data.messages.length > 0 ? response.data.messages : [messages[0]]);
    } catch (error) {
  ...
  const saveChat = async (updatedMessages, newSuggestions) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/chat/sessions/${currentChatId}`, {
    ...
    const processCellContent = (cell) => {
      ...
      while ((match = boldRegex.exec(cell)) !== null) {
        if (match.index > lastIndex) {
          parts.push(<span key={`text-${lastIndex}`}>{cell.substring(lastIndex, match.index)}</span>);
        }
      ...
      if (lastIndex < cell.length) {
        parts.push(<span key={`text-${lastIndex}`}>{cell.substring(lastIndex)}</span>);
      }
            ...
            {rows.map((row, rowIdx) => (
              ...
              >
                {row.map((cell, cellIdx) => (
                  <td key={cellIdx} className="px-4 py-3 text-sm text-gray-700 border-t border-gray-200">
                ...
                <div className="flex justify-start mt-3 animate-fadeIn">
                  <div className="max-w-3xl w-full" style={{ maxWidth: '85%' }}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      {msg.suggestedQuestions.slice(0, 4).map((question, idx) => (
                        <button
                          key={idx}
                        ...
                        >
                          <span className="font-medium group-hover:text-blue-600">{question}</span>
                        </button>
                      ))}
                    </div>
                  </div>
