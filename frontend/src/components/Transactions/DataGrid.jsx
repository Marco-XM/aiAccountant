      ...
      <div className="w-full overflow-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody className="text-ink">
            {safeRows.length === 0 ? (
              <tr>
                <td className="p-4 text-muted" colSpan={6}>No transactions to display</td>
              </tr>
            ) : (
              safeRows.map((r) => (
                <tr key={r._id} className="border-t border-theme hover:bg-surface transition-colors">
                  ...
                  <td className="px-2 py-2 text-right text-muted">{Number(r.taxAmount) ? Number(r.taxAmount).toFixed(2) : "—"}</td>
